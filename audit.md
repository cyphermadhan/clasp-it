...
- **#9 + analytics removed entirely.** `@vibesignals/observe` dropped
  from `website/package.json`; `website/src/analytics.js` deleted;
  every `observe.track(...)` call site cleaned out of Home, Upgrade,
  Verified, and the per-page main.jsx entry points. CLAUDE.md
  references updated. The "VibeSignals public key in client bundle"
  audit item is moot — no key is shipped any more.
- **#15 extension zip distribution removed.** Chrome Web Store is
  the canonical distribution channel; no need to host
  `clasp-it-extension.zip` ourselves. `server/public/downloads/`
  directory + the `build:extension` script in `server/package.json`
  are both gone.
...
### ~~9. VibeSignals public key committed in client bundle~~ ✅ MOOT — VibeSignals removed entirely from website
...
Removed alongside `clasp-it-setup.md`. The whole `downloads/` directory is gone, and the `build:extension` script in `server/package.json` was dropped. Extension distribution is via Chrome Web Store; no need to host the zip ourselves.
